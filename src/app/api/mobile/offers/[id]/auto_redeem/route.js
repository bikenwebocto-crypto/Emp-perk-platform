...
import { buildVirtualCard, VIRTUAL_CARD_AUTO_VERIFY } from "@/lib/virtual-card";
  ...
  try {
    ...
    if (enteredCode.toUpperCase() !== expectedCode.toUpperCase()) {
      void createAuditLog({
        actorType: "employee",
        actorId: employee.id,
        action: "AUTO_REDEEM_INVALID_CODE",
    ...
    // ── 6. Claim + capacity + create (atomic) ───────────────────
    let redemption;
    try {
    ...
    }).catch((e) => console.error("[AUTO_REDEEM] notify failed", e));

    void createAuditLog({
      actorType: "employee",
      actorId: employee.id,
      action: `REDEMPTION_CREATED_${VIRTUAL_CARD_AUTO_VERIFY}`,
    ...
    }).catch((e) => console.error("[AUTO_REDEEM] audit failed", e));

    const virtualCard = await buildVirtualCard({
      employeeId: employee.id,
    ...
    return NextResponse.json(
      {
        ...
        data: {
          ...
          redemptionCode: redemption.redemptionCode,
          virtualCard,
          instructions: redemptionConfig.instructions ?? null,
        },
      },
      { status: 201 },
    );
  } catch (error) {
    console.error("[AUTO_REDEEM] error", error);
